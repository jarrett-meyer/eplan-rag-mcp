...
def _execute_script(script_content: str, timeout: float = 30.0) -> dict:
    ...
    try:
        ...
        #
        # RegisterScript exists to install a script's PERSISTENT hooks
        # ([DeclareAction] / [DeclareEventHandler] / [DeclareMenu]). A one-shot
        # script like the ones generated here has only a [Start] method, and
        # EPLAN answers RegisterScript with a modal
        #   "The script does not contain attributes for loading."
        # dialog — one popup on the user's screen per MCP script call, for a
        # registration that was never needed: ExecuteScript compiles and runs
        # [Start] on its own. (bridge.py's header notes the same handshake
        # "without RegisterScript".)
